...
Texture::Texture(const std::string& text, const std::string& fontPath, int ptSize, const Color4f& textColor, const bool isWrapped)
	:m_Id{}
	, m_Width{ 10.0f }
	, m_Height{ 10.0f }
	, m_CreationOk{ false }
{
	CreateFromString(text, fontPath, ptSize, textColor, isWrapped);
}
...
void Texture::CreateFromString(const std::string& text, const std::string& fontPath, int ptSize, const Color4f& textColor, const bool isWrapped)
{
	m_CreationOk = true;
	...
	CreateFromString(text, pFont, textColor, isWrapped);
	TTF_CloseFont(pFont);
}

void Texture::CreateFromString(const std::string& text, TTF_Font* pFont, const Color4f& color, const bool isWrapped)
{
	m_CreationOk = true;
	if (pFont == nullptr)
	{
	...
	if (isWrapped) pLoadedSurface = TTF_RenderText_Blended_Wrapped(pFont, text.c_str(), textColor, 576);
	else pLoadedSurface = TTF_RenderText_Blended(pFont, text.c_str(), textColor);
