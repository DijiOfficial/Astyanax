...
bool EntityManager::GetMiniBossIsDead() const
{
	for (int i = 0; i < m_pEntity.size(); i++)
	{
		if (m_pEntity[i]->GetEnemyType() == Entity::EnemyType::miniBoss and (m_pEntity[i]->GetState() == Entity::ActionState::deactivated or m_pEntity[i]->GetState() == Entity::ActionState::dead))
			return true;
	}
	return false;
}
